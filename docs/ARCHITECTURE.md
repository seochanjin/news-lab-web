...
- `app/page.tsx`: 현재 `/` route, topic/article 목록 연결, mock keyword와 관련 기사 묶음 영역을 포함한다.
- `app/topics/`: Topic 목록 explorer route와 상세 dynamic route, route 상태를 포함한다.
- `components/layout/`: 홈과 상세 route가 재사용하는 공통 화면 구조를 포함한다.
- `components/topics/`: Topics API 목록과 카드 표시 컴포넌트를 포함한다.
- `lib/api/topics.ts`: Topics API response type, runtime validation, fetch 함수를 포함한다.
...
현재 Topics API client는 `lib/api/topics.ts`에 분리되어 있다. 기존 기사 API 연동은 `app/page.tsx`에 직접 구현되어 있다.
...
- `lib/api/topics.ts`에서 기존 backend `GET /topics` endpoint를 호출하고 response shape를 검사한다.
...
- Topic 목록 explorer는 Server Component에서 topic을 조회하고 Client Component에서 검색어와 `topic_date` 필터를 적용한다.
